...
const TENOR_API_KEY = 'AIzaSyAyimkuYQYF_y31kFrBDsLNDsHl5Uf7NZM';
...
export default function ChatScreen({ navigation, route }) {
  ...
  const fetchMessages = async () => {
    try {
      ...
      if (data.chat_bg) setChatBg(data.chat_bg);
    } catch (err) {
  ...
  const sendMessage = async () => {
    ...
    setInputText('');
    setSending(true);
    const temp = { id: Date.now(), sender_id: -1, text, sent_at: new Date().toISOString(), is_me: true, msg_type: 'text', group_invite: null };
  ...
  const handlePickImage = async () => {
    ...
    const result = await ImagePicker.launchImageLibraryAsync({
      mediaTypes: ImagePicker.MediaTypeOptions.Images, allowsEditing: false, quality: 0.5, base64: true,
    });
    if (!result.canceled) {
      const b64 = `data:image/jpeg;base64,${result.assets[0].base64}`;
      setSending(true);
      const temp = { id: Date.now(), sender_id: -1, text: b64, sent_at: new Date().toISOString(), is_me: true, msg_type: 'image', group_invite: null };
      setMessages(prev => [...prev, temp]);
      setTimeout(() => scrollRef.current?.scrollToEnd({ animated: true }), 80);
      try { await api.sendMessage(token, contactId, b64, 'image'); await fetchMessages(); }
  ...
  const openGifModal = () => { setGifModal(true); searchGifs(''); };
  ...
  const sendGif = async (gif) => {
    ...
    setGifModal(false);
    setSending(true);
    const temp = { id: Date.now(), sender_id: -1, text: url, sent_at: new Date().toISOString(), is_me: true, msg_type: 'gif', group_invite: null };
    setMessages(prev => [...prev, temp]);
    setTimeout(() => scrollRef.current?.scrollToEnd({ animated: true }), 80);
    try { await api.sendMessage(token, contactId, url, 'gif'); await fetchMessages(); }
  ...
  const stopRecording = async () => {
    ...
    try {
      ...
      setRecording(null);
      if (!uri) { Alert.alert('Error', 'Recording URI not found'); return; }
      // FIX: use top-level import instead of dynamic import
      const base64 = await FileSystem.readAsStringAsync(uri, { encoding: FileSystem.EncodingType.Base64 });
      const audioData = `data:audio/m4a;base64,${base64}`;
      setSending(true);
      const temp = { id: Date.now(), sender_id: -1, text: audioData, sent_at: new Date().toISOString(), is_me: true, msg_type: 'voice', group_invite: null, duration };
      setMessages(prev => [...prev, temp]);
      setTimeout(() => scrollRef.current?.scrollToEnd({ animated: true }), 80);
      try { await api.sendMessage(token, contactId, audioData, 'voice'); await fetchMessages(); }
  ...
  const handleSaveNickname = async () => {
    ...
    try { await api.setNickname(token, contactId, nn); } catch {}
  };

  // ── Block / Report / Delete ────────────────────────────────────────
  const handleBlock = () => {
    Alert.alert('🚫 Block user?', `You won't receive messages from ${contactName} anymore.`, [
      { text: 'Cancel', style: 'cancel' },
      { text: 'Block', style: 'destructive', onPress: async () => {
        try { await api.blockUser(token, contactId); setInfoModal(false); navigation.goBack(); }
  ...
  const handleReport = () => {
    Alert.alert('⚠️ Report user?', `Report ${contactName} for inappropriate behavior?`, [
      { text: 'Cancel', style: 'cancel' },
      { text: 'Report', style: 'destructive', onPress: async () => {
        try { await api.reportUser(token, contactId); setInfoModal(false); Alert.alert('✅ Reported', 'Thanks for letting us know.'); }
        catch (err) { Alert.alert('Error', err.message); }
      }},
    ]);
  };
  ...
  const handleDeleteChat = () => {
    Alert.alert('🗑 Delete chat?', `Clear all messages with ${contactName}?`, [
      { text: 'Cancel', style: 'cancel' },
      { text: 'Delete', style: 'destructive', onPress: async () => {
        try { await api.deleteChat(token, contactId); setInfoModal(false); navigation.goBack(); }
        catch (err) { Alert.alert('Error', err.message); }
      }},
    ]);
  };

  // ── Chat background — shared (saved to backend) ────────────────────
  const handleChangeBg = async () => {
    ...
    if (!result.canceled) {
      ...
      setChatBg(b64);
      setInfoModal(false);
      try { await api.setChatBackground(token, contactId, b64); }
      catch (err) { console.log('BG save err:', err.message); }
    }
  };
  ...
  const handleRemoveBg = async () => {
    setChatBg(null);
    setInfoModal(false);
    try { await api.setChatBackground(token, contactId, ''); } catch {}
  };
    ...
    try {
      ...
      if (action === 'accept') {
        Alert.alert('🎉 Joined!', `You're now in ${groupName}!`, [
          { text: 'Open group', onPress: () => navigation.navigate('GroupChat', { groupId, groupName, groupImage: groupImage || '', ownerId: null, token, displayName, profileImage: profileImage || '' }) },
  ...
  const formatTime = (t) => {
    if (!t) return '';
    try {
      // SQLite omits 'Z' — append it so JS treats as UTC, giving correct local time
  ...
  const renderMessage = (msg, i) => {
    ...
    if (msg.msg_type === 'group_invite' && msg.group_invite) {
      ...
      const isPend = inv.status === 'pending', acc = inv.status === 'accepted', isDel = inv.status === 'deleted' || inv.group_id === null;
      const gi = (inv.group_name || 'GR').slice(0,2).toUpperCase(), isLoad = respondingId === inv.invite_id;
      return (
        <View key={msg.id}>
          ...
          <View style={[styles.msgRow, msg.is_me && styles.msgRowMe]}>
            ...
            <View style={styles.inviteCard}>
              <View style={styles.inviteTop}>
                <View style={styles.inviteAvatar}>{inv.group_image ? <Image source={{ uri: inv.group_image }} style={styles.inviteAvatarImg} resizeMode="cover" /> : <Text style={styles.inviteAvatarText}>{gi}</Text>}</View>
                <View style={styles.inviteInfo}><Text style={styles.inviteTitle}>{msg.is_me ? 'You invited them to' : "You're invited to"}</Text><Text style={styles.inviteGroupName} numberOfLines={1}>{inv.group_name}</Text></View>
              </View>
              {isDel ? <View style={styles.inviteStatusRow}><Text style={styles.inviteStatusText}>🗑 This group no longer exists</Text></View>
               : msg.is_me ? <View style={styles.inviteStatusRow}><Text style={styles.inviteStatusText}>{acc ? '✓ They joined!' : inv.status==='declined' ? '✕ Declined' : '📨 Pending'}</Text></View>
               : isPend ? <View style={styles.inviteBtnRow}>
                  <TouchableOpacity style={[styles.joinBtn,isLoad&&{opacity:0.6}]} onPress={()=>handleGroupInviteRespond(inv.invite_id,'accept',inv.group_name,inv.group_id,inv.group_image)} disabled={isLoad} activeOpacity={0.85}>{isLoad?<ActivityIndicator color="#fff" size="small"/>:<Text style={styles.joinBtnText}>✓  Join</Text>}</TouchableOpacity>
                  <TouchableOpacity style={[styles.declineBtn,isLoad&&{opacity:0.6}]} onPress={()=>handleGroupInviteRespond(inv.invite_id,'decline',inv.group_name,inv.group_id,inv.group_image)} disabled={isLoad} activeOpacity={0.85}><Text style={styles.declineBtnText}>✕  Decline</Text></TouchableOpacity>
                </View>
               : <View style={styles.inviteStatusRow}><Text style={[styles.inviteStatusText,acc&&{color:'#22C55E'}]}>{acc?'✓ Joined!':'✕ Declined'}</Text></View>}
            </View>
          </View>
        </View>
      );
    ...
    if (msg.msg_type === 'image') return (
      <View key={msg.id}>
        ...
        <View style={[styles.msgRow, msg.is_me && styles.msgRowMe]}>
          {!msg.is_me && avatarEl}
          <TouchableOpacity style={[styles.imgBubble, msg.is_me && styles.imgBubbleMe]} onPress={() => setViewingImage(msg.text)} activeOpacity={0.9}>
            <Image source={{ uri: msg.text }} style={styles.msgImage} resizeMode="cover" />
            <View style={styles.imgTimeWrap}><Text style={styles.imgTime}>{formatTime(msg.sent_at)}</Text></View>
          </TouchableOpacity>
        </View>
      </View>
    );

    if (msg.msg_type === 'gif') return (
      <View key={msg.id}>
        {showTime && <Text style={styles.timeStamp}>{formatTime(msg.sent_at)}</Text>}
        <View style={[styles.msgRow, msg.is_me && styles.msgRowMe]}>
          {!msg.is_me && avatarEl}
    ...
    if (msg.msg_type === 'voice') {
      ...
      return (
        <View key={msg.id}>
          ...
          <View style={[styles.msgRow, msg.is_me && styles.msgRowMe]}>
            {!msg.is_me && avatarEl}
            <TouchableOpacity style={[styles.voiceBubble, msg.is_me ? styles.voiceBubbleMe : styles.voiceBubbleThem]} onPress={() => playVoice(msg)} activeOpacity={0.8}>
              <Text style={[styles.voiceIcon, msg.is_me && {color:'#fff'}]}>{isPlaying ? '⏸' : '▶'}</Text>
              <View style={styles.voiceWave}>{[...Array(18)].map((_,i)=><View key={i} style={[styles.voiceBar, msg.is_me&&styles.voiceBarMe, {height:6+Math.abs(Math.sin(i*0.8))*14}]}/>)}</View>
              <Text style={[styles.voiceDuration, msg.is_me&&{color:'rgba(255,255,255,0.7)'}]}>{formatDuration(msg.duration||0)}</Text>
            </TouchableOpacity>
          </View>
        </View>
      );
    ...
    <SafeAreaView style={styles.safe}>
      ...
      <Animated.View style={[styles.header, { opacity: fadeAnim }]}>
        ...
        <TouchableOpacity style={styles.headerCenter} onPress={() => setInfoModal(true)} activeOpacity={0.8}>
          <View style={styles.headerAvatar}>
            {profileImage ? <Image source={{ uri: profileImage }} style={styles.headerAvatarImg} /> : <Text style={styles.headerAvatarText}>{initials}</Text>}
          </View>
          <View>
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
          ...
          <ScrollView ref={scrollRef} style={styles.messageList} contentContainerStyle={styles.messageListContent} showsVerticalScrollIndicator={false} onContentSizeChange={() => scrollRef.current?.scrollToEnd({ animated: false })}>
            {messages.length === 0 && (
              <View style={styles.emptyChat}>
                <View style={styles.emptyChatAvatar}>{profileImage ? <Image source={{ uri: profileImage }} style={styles.emptyChatAvatarImg} /> : <Text style={styles.emptyChatAvatarText}>{initials}</Text>}</View>
        ...
        {isRecording ? (
          <View style={styles.recordingBar}>
            <TouchableOpacity onPress={cancelRecording} style={styles.cancelRecBtn}><Text style={styles.cancelRecText}>✕</Text></TouchableOpacity>
            <View style={styles.recordingIndicator}>
              <View style={styles.recDot} />
              <Text style={styles.recTime}>{formatDuration(recordingTime)}</Text>
              <Text style={styles.recLabel}>Recording...</Text>
            </View>
            <TouchableOpacity onPress={stopRecording} style={styles.sendRecBtn}><Text style={styles.sendRecText}>↑</Text></TouchableOpacity>
          </View>
        ) : (
          <View style={styles.inputBar}>
            <TouchableOpacity style={styles.attachBtn} onPress={handlePickImage} activeOpacity={0.8}><Text style={styles.attachBtnText}>🖼</Text></TouchableOpacity>
            <TouchableOpacity style={styles.attachBtn} onPress={openGifModal} activeOpacity={0.8}><Text style={styles.attachBtnText}>GIF</Text></TouchableOpacity>
            <View style={styles.inputWrap}>
              <TextInput style={styles.input} placeholder={`Message ${displayContactName.split(' ')[0]}...`} placeholderTextColor="rgba(107,107,138,0.5)" value={inputText} onChangeText={setInputText} multiline maxLength={500} autoCorrect={false} blurOnSubmit={false} />
            </View>
            {inputText.trim() ? (
              <TouchableOpacity style={[styles.sendBtn, sending && styles.sendBtnDisabled]} onPress={sendMessage} activeOpacity={0.85} disabled={sending}>
                {sending ? <ActivityIndicator color="#fff" size="small" /> : <Text style={styles.sendBtnText}>↑</Text>}
              </TouchableOpacity>
            ) : (
              <TouchableOpacity style={styles.micBtn} onPress={startRecording} activeOpacity={0.8}><Text style={styles.micBtnText}>🎤</Text></TouchableOpacity>
            )}
          </View>
        )}
      ...
      <Modal visible={!!viewingImage} transparent animationType="fade" onRequestClose={() => setViewingImage(null)}>
        <View style={styles.imageViewerOverlay}>
          ...
          {viewingImage && <Image source={{ uri: viewingImage }} style={styles.imageViewerImg} resizeMode="contain" />}
        </View>
      </Modal>
          ...
          <View style={styles.infoSheet}>
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
              <View style={styles.infoProfileSection}>
                <View style={styles.infoAvatar}>{profileImage ? <Image source={{ uri: profileImage }} style={styles.infoAvatarImg} /> : <Text style={styles.infoAvatarText}>{initials}</Text>}</View>
              ...
              <TouchableOpacity style={styles.infoRow} onPress={() => { setNicknameInput(nickname); setInfoModal(false); setNicknameModal(true); }} activeOpacity={0.75}>
                ...
                <View style={{ flex:1 }}><Text style={styles.infoRowTitle}>Nickname</Text><Text style={styles.infoRowSub}>{nickname || 'Set a nickname only you can see'}</Text></View>
              ...
              <TouchableOpacity style={styles.infoRow} onPress={() => { setInfoModal(false); handleChangeBg(); }} activeOpacity={0.75}>
                ...
                <View style={{ flex:1 }}><Text style={styles.infoRowTitle}>Chat Background</Text><Text style={styles.infoRowSub}>{chatBg ? 'Custom background set (visible to both)' : 'Choose from your gallery'}</Text></View>
                <Text style={styles.infoRowArrow}>›</Text>
              </TouchableOpacity>
              {chatBg && (
                <TouchableOpacity style={styles.infoRow} onPress={handleRemoveBg} activeOpacity={0.75}>
                  <View style={styles.infoRowIcon}><Text style={{ fontSize:20 }}>🔄</Text></View>
                  <View style={{ flex:1 }}><Text style={styles.infoRowTitle}>Remove Background</Text><Text style={styles.infoRowSub}>Go back to default</Text></View>
                </TouchableOpacity>
              )}
              <View style={styles.infoDivider} />
              ...
              <TouchableOpacity style={styles.infoRow} onPress={() => { setInfoModal(false); handleBlock(); }} activeOpacity={0.75}>
                <View style={styles.infoRowIcon}><Text style={{ fontSize:20 }}>🚫</Text></View>
                <View style={{ flex:1 }}><Text style={[styles.infoRowTitle,{color:'#EF4444'}]}>Block</Text><Text style={styles.infoRowSub}>Stop receiving messages from them</Text></View>
                <Text style={styles.infoRowArrow}>›</Text>
              </TouchableOpacity>
              <TouchableOpacity style={styles.infoRow} onPress={() => { setInfoModal(false); handleReport(); }} activeOpacity={0.75}>
                <View style={styles.infoRowIcon}><Text style={{ fontSize:20 }}>⚠️</Text></View>
                <View style={{ flex:1 }}><Text style={[styles.infoRowTitle,{color:'#F97316'}]}>Report</Text><Text style={styles.infoRowSub}>Report inappropriate behavior</Text></View>
                <Text style={styles.infoRowArrow}>›</Text>
              </TouchableOpacity>
              <TouchableOpacity style={styles.infoRow} onPress={() => { setInfoModal(false); handleDeleteChat(); }} activeOpacity={0.75}>
                <View style={styles.infoRowIcon}><Text style={{ fontSize:20 }}>🗑</Text></View>
                <View style={{ flex:1 }}><Text style={[styles.infoRowTitle,{color:'#EF4444'}]}>Delete Chat</Text><Text style={styles.infoRowSub}>Clear messages (friend stays)</Text></View>
                <Text style={styles.infoRowArrow}>›</Text>
              </TouchableOpacity>
            </ScrollView>
            <TouchableOpacity style={styles.closeBtn} onPress={() => setInfoModal(false)}><Text style={styles.closeBtnText}>Close</Text></TouchableOpacity>
          </View>
        </View>
      </Modal>
      ...
      <Modal visible={nicknameModal} transparent animationType="slide" onRequestClose={() => setNicknameModal(false)}>
        <View style={styles.modalOverlay}>
          <View style={styles.smallSheet}>
            ...
            <TextInput style={styles.nicknameInput} placeholder={contactName} placeholderTextColor="rgba(107,107,138,0.4)" value={nicknameInput} onChangeText={setNicknameInput} autoCapitalize="words" autoFocus maxLength={30} />
            <TouchableOpacity style={styles.btnPrimary} onPress={handleSaveNickname} activeOpacity={0.85}><Text style={styles.btnPrimaryText}>Save nickname ✦</Text></TouchableOpacity>
            {nickname ? <TouchableOpacity style={styles.clearNicknameBtn} onPress={() => { setNicknameInput(''); setNickname(''); setNicknameModal(false); try { api.setNickname(token, contactId, ''); } catch {} }} activeOpacity={0.75}><Text style={styles.clearNicknameBtnText}>Remove nickname</Text></TouchableOpacity> : null}
            <TouchableOpacity style={styles.closeBtn} onPress={() => setNicknameModal(false)}><Text style={styles.closeBtnText}>Cancel</Text></TouchableOpacity>
          </View>
        </View>
      </Modal>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.gifSheet}>
            ...
            <View style={styles.gifSearchBar}>
              ...
              <TextInput style={styles.gifSearchInput} placeholder="Search GIFs..." placeholderTextColor="rgba(107,107,138,0.5)" value={gifSearch} onChangeText={t => { setGifSearch(t); searchGifs(t); }} autoCorrect={false} />
            </View>
            {gifLoading ? <ActivityIndicator size="large" color="#7B5EA7" style={{ marginTop:20 }} /> : (
              <FlatList data={gifs} numColumns={2} keyExtractor={item => item.id} style={{ marginTop:8 }} contentContainerStyle={{ gap:6, paddingHorizontal:4 }} columnWrapperStyle={{ gap:6 }}
                renderItem={({ item }) => {
                  const preview = item.media_formats?.tinygif?.url || item.media_formats?.gif?.url;
                  return <TouchableOpacity onPress={() => sendGif(item)} activeOpacity={0.8} style={styles.gifThumb}>{preview ? <Image source={{ uri: preview }} style={styles.gifThumbImg} resizeMode="cover" /> : null}</TouchableOpacity>;
                }}
              />
            )}
            <TouchableOpacity style={styles.closeBtn} onPress={() => setGifModal(false)}><Text style={styles.closeBtnText}>Cancel</Text></TouchableOpacity>
          </View>
        </View>
      </Modal>
    </SafeAreaView>
...
const styles = StyleSheet.create({
  ...
  chatBgImage: { position:'absolute', top:0, left:0, right:0, bottom:0, opacity:0.45, zIndex:0 },
  header: { flexDirection:'row', alignItems:'center', paddingHorizontal:16, paddingVertical:10, borderBottomWidth:1, borderBottomColor:'rgba(107,107,138,0.08)', backgroundColor:'rgba(250,250,248,0.96)', gap:10, zIndex:2 },
  backBtn: { padding:4 },
  ...
  timeStamp: { textAlign:'center', fontSize:11, color:'rgba(107,107,138,0.7)', marginVertical:8, backgroundColor:'rgba(250,250,248,0.8)', alignSelf:'center', paddingHorizontal:10, paddingVertical:3, borderRadius:10 },
  ...
  msgBubbleThem: { backgroundColor:'rgba(240,239,248,0.95)', borderBottomLeftRadius:4 },
  msgText: { fontSize:15, color:'#1A1A2E', lineHeight:21 },
  msgTextMe: { color:'#fff' },
  imgBubble: { borderRadius:16, overflow:'hidden', borderWidth:1.5, borderColor:'rgba(107,107,138,0.15)' },
  imgBubbleMe: { borderColor:'rgba(26,26,46,0.25)' },
  msgImage: { width:210, height:210 },
  imgTimeWrap: { position:'absolute', bottom:0, left:0, right:0, paddingVertical:6, paddingHorizontal:10, backgroundColor:'rgba(0,0,0,0.35)', alignItems:'flex-end' },
  imgTime: { fontSize:10, color:'#fff', fontWeight:'600' },
  ...
  voiceBubbleThem: { backgroundColor:'rgba(240,239,248,0.95)' },
  ...
  inputBar: { flexDirection:'row', alignItems:'flex-end', paddingHorizontal:10, paddingVertical:10, borderTopWidth:1, borderTopColor:'rgba(107,107,138,0.08)', gap:6, backgroundColor:'rgba(250,250,248,0.97)', zIndex:2 },
  ...
  recordingBar: { flexDirection:'row', alignItems:'center', paddingHorizontal:16, paddingVertical:12, borderTopWidth:1, borderTopColor:'rgba(107,107,138,0.08)', backgroundColor:'rgba(250,250,248,0.97)', gap:12, zIndex:2 },
  ...
  inviteCard: { width:240, backgroundColor:'rgba(255,255,255,0.97)', borderRadius:16, borderWidth:1.5, borderColor:'rgba(123,94,167,0.2)', overflow:'hidden', shadowColor:'#7B5EA7', shadowOpacity:0.08, shadowRadius:8, elevation:2 },
  ...
  imageViewerClose: { position:'absolute', top:50, right:20, width:40, height:40, borderRadius:20, backgroundColor:'rgba(255,255,255,0.15)', alignItems:'center', justifyContent:'center', zIndex:10 },
  imageViewerCloseText: { color:'#fff', fontSize:20, fontWeight:'700' },
  imageViewerImg: { width:SCREEN_WIDTH, height:SCREEN_HEIGHT * 0.8 },
